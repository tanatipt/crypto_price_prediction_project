...
def plot_year_target(btc_data):
    """

    Plotting the closing price for each day throughout the year to determine the range of years our neural network should
    focus on to learn the trend. Results have shown that the BTC-USD price have risen dramatically after 2020 ; hence, we 
    ...
    Args:
        btc_data (Pandas Dataframe): A Dataframe containing the target and features for each date.
    """
    ...
    for year in ['2018', '2019', '2020', '2021', '2022', '2023']:
        ...
        end_year = year + "-12-31"
        # Indexing the BTC-USD information for all the days within a single year
        year_data = btc_data[(btc_data['Date'] >= start_year)
                             & (btc_data['Date'] <= end_year)]
        print(year)
        print(year_data["Target"])
        year_target = year_data['Target']
        plt.plot(list(year_target.index), year_target, label=year)

    plt.xlabel("Date")
    plt.ylabel("BTC-USD Price")
    plt.title("BTC-USD Closing Price From 2018-2023")
    plt.legend()
    plt.show()


def plot_year_summary(btc_data):
    """
...
def plot_summary(btc_data):
    ...
    Args:
        btc_data (Pandas Dataframe): A Dataframe containing the target and features for each date.
    """
    feature_data = btc_data[btc_data["Date"]
...
def plot_feature_distribution(btc_data):
    """
...
plot_year_target(btc_data)
plot_year_summary(btc_data)
plot_summary(btc_data)
plot_year_trend(btc_data)
plot_feature_distribution(btc_data)
