...
class MyHyperModel(kt.HyperModel):
    ...
    def build(self, hp):
        ...
        This function builds a specific architecture of the LSTM neural network for BTC-USD price prediction as well as 
        ...
        # Defining the number of hidden units in the 1st LSTM hidden cell
        ...
        # Specifying the L1 regularisation hyperparameter at each layer to simultaneously perform feature selection while training the network
        alpha = hp.Float('alpha', min_value=0, max_value=0.1, step=0.01)

        regulariser = L1(l1=alpha)

        input = Input(shape=(7, 34))

        # Defining the 1st LSTM layer with varying number of hidden units
        lstm_1 = LSTM(units=hp_lstm_1,
                      kernel_initializer=weight_initialiser,  kernel_regularizer=regulariser)(input)
        ...
        dense_1 = Dense(units=hp_dense_1,
                        activation=activation_function, kernel_initializer=weight_initialiser,  kernel_regularizer=regulariser)(dropout_1)
        dropout_2 = Dropout(rate=dropout_rate)(dense_1)
        dense_2 = Dense(units=hp_dense_2,
                        activation=activation_function, kernel_initializer=weight_initialiser,  kernel_regularizer=regulariser)(dropout_2)
        dropout_3 = Dropout(rate=dropout_rate)(dense_2)
        ...
        output_reg = Dense(1,  kernel_initializer=weight_initialiser,
                           activation="linear",  kernel_regularizer=regulariser)(dropout_3)

        # Configuring the Adam optimizer object, which is the algorithm for SGD
